 ...
 * 演示不同的转场时长效果
 */
export async function example3c_customTransitionDuration(composer: VideoComposer) {
  // 快速转场（0.5 秒）
...
export async function example7_completeWorkflow(composer: VideoComposer) {
  ...
  // 步骤 4: 清理临时文件
  console.log('\n步骤 4: 清理临时文件...')
  await composer.cleanup(['merged.mp4', 'final_1.mp4', 'final_2.mp4', 'final_3.mp4'])
  console.log('✓ 清理完成')
