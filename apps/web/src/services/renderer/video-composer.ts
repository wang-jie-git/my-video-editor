...
export class VideoComposer {
  ...
  async concatWithTransitions(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : '转场合并失败'
      onProgress?.({
        phase: 'error',
        progress: 0,
        completed: 0,
  ...
  async splitVideo(
    ...
    const { outputPrefix, splitPoints, includeAudio = true, format = 'mp4' } = options
  ...
  async trimVideo(
    ...
    const { outputFile, startTime, endTime, reencode = false, format = 'mp4' } = options
  ...
  private buildMergeArgs(params: {
    ...
    if (reencode) {
      // 重新编码模式（支持转场，但速度慢）
      const filterComplex = this.buildConcatFilter(inputFiles.length, includeAudio)
   ...
   * 构建转场滤镜（xfade）
   */
  private buildTransitionFilter(inputFiles: string[], transitions: Transition[]): string {
  ...
  async cleanup(files?: string[]): Promise<void> {
    try {
      if (files) {
        await Promise.all(files.map((file) => this.ffmpegService.deleteFile(file)))
      } else {
        // 清理所有临时文件
